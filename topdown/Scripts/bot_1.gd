...
func get_input(): #Get the navigation agent position and find out if the bot should attack
	if player1_pos.length() > 2000 and player2_pos.length() > 2000 and bot2_pos.length() > 2000:
		...
		else: #if player1 and player2 isn't closest, bot2 has to be
			navigation_agent.target_position = bot2_pos
			#Set bot2 as the target
			if distance_to_bot2 < 70:
				bot_attack_or_guard_input = 1
			#if bot2 is closest and within 70 px, start attack
			elif distance_to_bot2 < 100:
				bot_attack_or_guard_input = -1
			#if bot2 is closest and within 100 to 70 px, start guard
			else:
				bot_attack_or_guard_input = 0
			#if bot2 is closest but over 100 px, dash towards the player
...
func enter_dead_state() -> void: #If hp is below 0
	...
	emit_signal("bot1_dead") #Send out a singal to the level script so that the level knows it is dead
